...
export default function Party() {
  ...
  async function getUser() {
    try{
      ...
      let allVal = await axios.get("http://localhost:3000/delta/user/getUsers");

     

      let secondUser = prompt(`Enter The Name of User(correctly) :`);

      const partyOne = await axios.post("http://localhost:3000/delta/user/oneUser", {
        username: secondUser,
      });
      
      const getSongs = await axios.get("http://localhost:3000/delta/user/getSongs");

      let array1 = val.data.favSongs;
      let array2 = partyOne.data.favSongs;
      let combined = []
      ...
      setUser(combined);
      setPlaylist(`Party Mode with ${secondUser}`)
  ...
  return (
    <div className="bg-gradient-to-r from-indigo-500 from-10% via-sky-500 via-30% to-emerald-500 to-90% flex flex-col justify-center items-center p-5">
      <div className="m-3">
      <p className="m-5 font-serif size-9 inline">{playlist}</p>
      <button
